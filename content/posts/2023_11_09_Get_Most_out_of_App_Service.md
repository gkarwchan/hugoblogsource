...
But wait, is the storage url still accessible?  
The answer is Yes.  

### What to do with the storage url?
But we don't want to let the storage URL still accessible, and if a visitor visited the storage URL we need to forward to the new custom domain.  
And this has to happen from the storage where you store your static contents. Github pages, Azure blog storage, and Azure static website, all provide way to configure the custom domain, so in case a visitor visited that url, they will be forwarded to the new custom domain.
